            ...
            columns: [
                ...
                },
                {
                    data: "action", render: function (data, type, row) {
                        ...
                        switch (parseInt(row.bl_status)) {
                            case 0:
                                buttonObj += '<button class="btn btn-dark btn-viewBill" value="' + row.id + '">View</button>';
                                break;
                            case 1:
                                if (parseInt(row.bl_auth_status) == 0) {
                                    buttonObj += '<div class="d-grid gap-2"><button class="btn btn-dark btn-viewBill" value="' + row.id + '">View</button><button class="btn btn-secondary text-white btn-checkAuth" value="' + row.id + '">Mark as Checked</button><button class="btn btn-danger text-white btn-cancelBill" value="' + row.id + '">Cancel</button></div>';
                                } else {
                                    buttonObj += '<div class="d-grid gap-2"><button class="btn btn-dark btn-viewBill" value="' + row.id + '">View</button><button class="btn btn-danger text-white btn-cancelBill" value="' + row.id + '">Cancel</button></div>';
                                }                                
                                break;
                            case 2:
                                if (parseInt(row.bl_auth_status) == 0) {
